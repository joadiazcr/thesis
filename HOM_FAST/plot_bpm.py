...
BPMs = ['B418PH', 'B418PV', 'B440PH', 'B440PV', 'B441PH', 'B441PV', 'B444PH', 'B444PV']
num_BPMs = len(BPMs)
num_rows = 2
num_cols = 4
...
axs.set_ylabel('Position ' + r'$\mathbf{[\mu m]}$', fontsize=16, fontweight='bold')
secax = axs.secondary_xaxis('top', functions=(bunch2us, us2bunch))
secax.set_xlabel('Time ' + r'$\mathbf{[\mu s]}$', fontsize=16, fontweight='bold')
secax.set_xticklabels(['0','0','2','4','6','8','10','12','14','16'],fontsize=14, fontweight='bold')
...
for bpm in range(0,num_BPMs):
    ...
    for v125 in range(0, len(df['V125'])):
        ...
        bpm_mean_mean = df[bpm_name + '_mean'][v125]
        label = (str(df['V125'][v125]) + ' A')
        axs[int(bpm / num_cols), bpm % num_cols].plot(bpm_mean - bpm_mean_mean, label=label)
    axs[int(bpm / num_cols), bpm % num_cols].grid(True)
    title = (bpm_name)
    axs[int(bpm / num_cols), bpm % num_cols].set_title(title, fontsize=18)
    axs[int(bpm / num_cols), bpm % num_cols].tick_params(axis='both', which='major', labelsize=16)
    if int(bpm / 4) == 1:
