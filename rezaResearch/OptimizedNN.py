...
def build_model():
  ...
  optimizer = tf.keras.optimizers.RMSprop(0.001) # minimizing the sqr(y-y_est). This is optimizing via gradient decent with step 0.001

  model.compile(loss='mean_squared_error', # We want to minimize this. Which in this case is the MSE
                optimizer=optimizer, 
                metrics=['mean_squared_error','mean_absolute_percentage_error']) # To see how good the stimation is
  return model
model = build_model()
...
history = model.fit(normed_train_data, train_labels, epochs=EPOCHS,
                    validation_split = 0.2, verbose=0, callbacks=[early_stop, PrintDot()]) # TRAINING OF NEURAL NETWORK!!! (and validation)

plot_history(history)
## Testing error
loss, mse, mape = model.evaluate(normed_test_data, test_labels, verbose=0) # Test of the Neual Network. outputs are metrics

print("Testing set Mean Abs Error: {:5.2f} Energy".format(mse))

## Make prediction

test_predictions = model.predict(normed_test_data).flatten() # Test of the dataset. outputs are predictions (in this case energy)
...
plt.hist(error, bins = 25) # This gives you a gaussian
