...
def test_error_integral():
    ...
    plt.plot(t, np.ones(len(t)) * sp, '--', label='Set Point')
    plt.plot(t, vol, label='Signal')
    plt.plot(t, ei[0:len(t)], label='Error integral')
    plt.show()
...
def log(data_dir, settings):
    ...
    print("Simulation settings saved in ", data_dir, "/simulation_settings.json")
    return 0
...
if __name__ == "__main__":
    ...
    #stable_gbws=[70000, 40000]
    #control_zero=[17500, 10000]
    #stable_gbws=[50000]
    #control_zero=[12500]
    ...
    if not args.path:
        ...
        for s_gbw in stable_gbws:
            ...
            t, cav_v, sp, fwd, Kp_a, Ki_a, e, ie = cavity_step_response.cavity_step(tf, nom_grad, feedforward, noise, noise_psd,\
    ...
    for p in range(len(stable_gbws)):
        ...
        axs[1, 0].plot(t, nom_grad*np.ones(len(t))*(1 + amp_stability), linewidth=2, color='m')
        axs[1, 0].plot(t, nom_grad*np.ones(len(t))*(1 - amp_stability), linewidth=2, color='m')
        ...
        axs[1, 1].plot(t, np.ones(len(t)) * (sp + ssa_limit), linewidth=2, color='m')
        axs[1, 1].plot(t, np.ones(len(t)) * (sp - ssa_limit), linewidth=2, color='m')
        ...
        axs[1, 2].plot(t[steady_state_start-1:-1], np.abs(ie_s[p][0:len(t)]), label=stable_gbws[p])
