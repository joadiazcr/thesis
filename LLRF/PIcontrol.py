...
def find_zero_crossing(vector, f_vector):
    """
    Find the 0-crossing given a vector in frequency
    domain and the associated frequency vector.
    """
    signs = np.sign(vector)
    signs[signs == 0] = -1
    zero_crossing = np.where(np.diff(signs))[0]
    return vector[zero_crossing], f_vector[zero_crossing], zero_crossing
...
def lpf(f, wc, cavity):
    ...
    plt.subplot(2, 1, 1)
    r_fc = np.around(fc, 2)
    tlt = "Low Pass Filter amplitude and phase response \
    with cut-off frequency = %s" % r_fc
    plt.title(tlt)
    plt.plot(f, gain)
    plt.xscale('log')
...
def pid(f, s, wc, cavity, ssa_bw=53000, stable_gbw=20000,
        control_zero=5000, conf='nominal', plot=0):
    ...
    if plot == 1:
        ...
        plt.semilogx(f, 20.0*np.log10(np.abs(plant)), linewidth="3",
                     label='Plant')
        plt.semilogx(f, 20.0*np.log10(np.abs(controller)), linewidth="3",
                     label='Controller')
        plt.semilogx(f, 20.0*np.log10(np.abs(loop)), linewidth="3",
                     label='Loop')
        ...
        plt.annotate(s='', xy=(f_180, gain[f_180_index]),
                     xytext=(f_180, unity[f_180_index]-1),
                     arrowprops=dict(arrowstyle='<->'))
        f_c_text = r'$f_{\rm c} \approx %d\,{\rm kHz}$' % np.ceil(f_c*1e-3)
        plt.annotate(f_c_text, xy=(f_c, 1), xytext=(f_c*1.2, 25),
        ...
        f_180_text = r'$f_{\rm 180} \approx %d\,{\rm kHz}$'\
                     % int(np.around(f_180*1e-3))
        plt.annotate(f_180_text, xy=(f_180, -180), xytext=(f_180*1.2, -100),
                     arrowprops=dict(facecolor='black', shrink=0.05))
...
if __name__ == "__main__":
    desc = 'Low Pass Filter and PID control concepts'
    hlp = 'Function: lpf (low-pass filter) or \
           pid (pid controller) or \
           swp (gain sweep) or \
           ip (impulse response)'
    ...
    if args.f == 'lpf':
        ...
        for ff in [0.1, 2, 3, 10]:
            ...
            ssa_bw = data['ssa_bw']
            control_zero = data['control_zero']
            pid_results = pid(f, s, wc, cavity, ssa_bw, stable_gbw,
                              control_zero, conf, 0)
            plt.semilogx(f, 20.0*np.log10(np.abs(pid_results[2])),
                         linewidth="3", label='%s *k_p' % ff)
        unity = np.ones(len(s))
